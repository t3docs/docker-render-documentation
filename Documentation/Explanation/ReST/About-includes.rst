...
include
   `include
   <http://docutils.sourceforge.net/docs/ref/rst/directives.html#including-an-external-document-fragment>`__
...
Both directives represent a potential security hole. They can be disabled with
the Docutils "file_insertion_enabled" runtime setting.

In our container the `.. include::` and the `.. literalinclude::` are possible
and **enabled**.
...
Relative paths represent a security hole.
...
outside the project is referenced the rendering will be aborted. A message is
shown on stdout in the terminal window.
